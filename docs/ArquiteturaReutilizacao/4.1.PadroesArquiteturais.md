...
Esse padrão foi selecionado por se tratar de uma aplicação web em que existem várias formas de interagir e visualizar os dados do sistema, porém foi feita uma adaptação para o projeto, onde toda a camada de Visão ficou responsável pelas atualizações de interface, em que ela e a camada de controle se comunicam por meio do estilo arquitetural **cliente-servidor**, por meio de requisições HTTP.

### Cliente-Servidor

Neste estilo as funções de um sistema são divididas entre dois componentes: o cliente, que faz as solicitações, e o servidor, que processa essas solicitações e fornece os recursos ou serviços necessários. Para esse projeto, esse estilo será utilizado nas interações entre a camada de visão e de controle. A camada de **visão (cliente)** fará requisições para a **controladora (servidor)** , que por sua vez processará essas requisições e retornará as respostas necessárias para a atualização da interface. Essa escolha foi feita por restrições das tecnologias utilizadas no projeto, em que as atualizações de interface são feitas diretamente com os elementos gráficos (UI).

Dessa forma, esse estilo arquitetural foi utilizado nas interações entre o banco de dados e a aplicação. Onde as funções de um sistema são divididas entre dois componentes: o **cliente** que é responsável por fazer as solicitações, e o **servidor**, que processa essas solicitações e fornece os recursos ou serviços necessários. Para o projeto, o servidor será responsável por gerenciar as requisições de dados do banco de dados e fornecer as informações necessárias para a aplicação (cliente).
...
Para a comunicação entre o cliente e o servidor foi utilizado outro estilo arquitetural chamado REST. O REST é um estilo arquitetural para a criação de serviços web criando um padrão de comunicação que segue os princípios REST com uso de verbos HTTP entre outros recursos.
