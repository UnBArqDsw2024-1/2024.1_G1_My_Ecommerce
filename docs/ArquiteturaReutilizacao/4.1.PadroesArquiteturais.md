...
Esse padrão foi selecionado por se tratar de uma aplicação web em que tem várias formas de interagir e visualizar com os dados do sistema, porém foi feita uma adaptação para o projeto, onde todas a camada de Visão ficou responsável pelas atualizações de interface, em que ela e a camada de controle se comunicam por meio do estilo arquitetural **cliente-servidor**, por meio de requisições HTTP.
